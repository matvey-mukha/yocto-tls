...
TEST_CASE("Fixed vector handshake and message exchange on the client side", "[fixed vector]")
{
    ...
    yocto_tls_client_generate_client_hello(&context, buffer, &client_hello_length, binder_key, handshake_secret,
                                           identity);
    REQUIRE_THAT(vector<uint8_t>(buffer, buffer + client_hello_length),
                 Catch::Matchers::Equals(vector<uint8_t>(client_hello, client_hello + sizeof(client_hello))));
    ...
    REQUIRE_THAT(vector<uint8_t>(buffer, buffer + client_finished_length),
                 Catch::Matchers::Equals(vector<uint8_t>(client_finished, client_finished + sizeof(client_finished))));
    yocto_tls_encrypt(&context, true, (uint8_t *)plain_text, sizeof(plain_text), buffer);
    REQUIRE_THAT(vector<uint8_t>(buffer, buffer + sizeof(plain_text) + YOCTO_TLS_ENCRYPTED_DATA_LENGTH_DELTA),
                 Catch::Matchers::Equals(
                     vector<uint8_t>(client_data_message, client_data_message + sizeof(client_data_message))));
    res = yocto_tls_decrypt(&context, true, (uint8_t *)server_data_message, sizeof(server_data_message), buffer);
    REQUIRE(res == true);
}

TEST_CASE("Fixed vector handshake and message exchange on the server side", "[fixed vector]")
{
    ...
    REQUIRE_THAT(vector<uint8_t>(buffer, buffer + server_hello_length),
                 Catch::Matchers::Equals(vector<uint8_t>(server_hello, server_hello + sizeof(server_hello))));
    yocto_tls_server_generate_server_encrypted_extensions(&context, buffer, &server_encrypted_extensions_length);
    REQUIRE_THAT(vector<uint8_t>(buffer, buffer + server_encrypted_extensions_length),
                 Catch::Matchers::Equals(vector<uint8_t>(server_enc_ext, server_enc_ext + sizeof(server_enc_ext))));
    yocto_tls_server_generate_server_finished(&context, buffer, &server_finished_length);
    REQUIRE_THAT(vector<uint8_t>(buffer, buffer + server_finished_length),
                 Catch::Matchers::Equals(vector<uint8_t>(server_finished, server_finished + sizeof(server_finished))));
    ...
    REQUIRE_THAT(vector<uint8_t>(buffer, buffer + sizeof(plain_text) + YOCTO_TLS_ENCRYPTED_DATA_LENGTH_DELTA),
                 Catch::Matchers::Equals(
                     vector<uint8_t>(server_data_message, server_data_message + sizeof(server_data_message))));
